...
| Mobile setup | Replaced the dark-theme engine card and duplicate remote actions with one MADE surface; a saved Session home now becomes **Reconnect <host>** with its address prefilled instead of falsely asking the user to connect compute again | The setup blocker distinguishes first-time setup from session-only credential renewal |
